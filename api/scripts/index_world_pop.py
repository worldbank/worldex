...
def main():
    ...
    with Session() as sess:
        result = sess.execute(text("SELECT COUNT(*) FROM datasets WHERE source_org = 'WorldPop'"))
        existing_dataset_count = result.one()[0]
        dirs = s3.ls("s3:///worldex-temp-storage/indexes/worldpop/")
        print(f"Skipping {existing_dataset_count} datasets")
        # best effort to skip existing datasets assuming ls order has not changed
        for dir in dirs[existing_dataset_count:existing_dataset_count+INCREMENTS]:
            ...
            try:
                with s3.open(f"s3://{dir}/metadata.json") as f:
                    ...
                    sess.add(dataset)
                    sess.flush()
                with s3.open(f"s3://{dir}/h3-compact.parquet") as f:
